...
const Vitaltest = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:8000/api/vital/getV/${pid}`);
      setVitalData({ finalconclution: response.data.finalconclution });
    } catch (err) {
    ...
    <div className="container">
      ...
      {patientData && (
        <div className="patient-data">
          <img src={`http://localhost:8000/${patientData.photo}`} alt="Patient" style={{ width: '200px', height: '200px' }} />
      ...
      {labData && (
       <div className="form-group">
          <h3>Lab Results</h3>
          <p><strong>Conclusion Status:</strong> {labData.coclusionStatus}</p>
        </div>
      )}
...
const Test = () => {
  const [patientId, setPatientId] = useState('');
  const [vital, setFinalconclution] = useState(null);
  const [xray, setConclusion] = useState(null);
  const [labratory, setCoclusionStatus] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const patientDataResponse = await axios.get(`http://localhost:8000/api/patient/get/${patientId}`);
        setData({
          photo: patientDataResponse.data.photo,
          passPort: patientDataResponse.data.passPort,
          fname: patientDataResponse.data.fname,
          mname: patientDataResponse.data.mname,
          lname: patientDataResponse.data.lname,
        });

        const vitalResponse = await axios.get(`http://localhost:8000/api/vital/getV/${patientId}`);
        setFinalconclution({finalconclution:vitalResponse.data.finalconclution});

        const xrayResponse = await axios.get(`http://localhost:8000/api/xray/getX/${patientId}`);
        setConclusion({conclusion:xrayResponse.data.conclusion});

        const labResponse = await axios.get(`http://localhost:8000/api/labresult/getL/${patientId}`);
        setCoclusionStatus({coclusionStatus:labResponse.data.coclusionStatus});
  ...
  return (
    <div className="container">
      ...
      {data && (
         <div className="form-group">
          <img src={`http://localhost:8000/${data.photo}`} alt="Patient" style={{ width: '200px', height: '200px' }} />
          <p><strong>Passport Number:</strong> {data.passPort}</p>
 ...
 {vital && <p><strong>Final Conclusion:</strong> {vital.finalconclution}</p>}
 {xray && <p><strong>Conclusion Status:</strong> {xray.conclusion}</p>}
 {labratory &&<p><strong>Conclusion:</strong> {labratory.coclusionStatus}</p>}
