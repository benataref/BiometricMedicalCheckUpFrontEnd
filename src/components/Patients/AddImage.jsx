...
export default function AddImage() {
    ...
    const handleCaptureFingerprint = async () => {
        try {
          const response = await axios.get('http://localhost:8000/api/patient/handleCaptureFingerprint');
          setfingerprintData(response.data.data); // Assuming the API returns the fingerprint data
    ...
    const handleSubmit = async (e) => {
       ...
       // console.log('Captured fingerprint data:', fingerprintData);
        try {
            ...
            if (fingerprintData) {
              formData.append('fingerKey', fingerprintData); // Append fingerprint data
           }
            for (const [key, val] of Object.entries(value)) {
              formData.append(key, val);
           }
    
            const response = await axios.post('http://localhost:8000/api/patient/create5', formData, {
                headers: {
    ...
    return (
        <>
          ...
          <button type="button" onClick={handleCaptureFingerprint} className="btn btn-secondary">
            Capture Fingerprint
          </button>
          <input
            type="text"
            ...
            value={value.fingerKey}
            readOnly
            className="form-control mt-2"
            placeholder="Fingerprint data will appear here"
          />   
       </div>
